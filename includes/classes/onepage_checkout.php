...
class osC_onePageCheckout {
    ...
    if (!tep_session_is_registered('onepage') || $forceReset === true){
      ...
      $onepage = array(
        ...
        'customer'       => array(
          ...
          'format_id' => '',  'telephone' => '', 'email_address' => '', 'password' => '', 'newsletter' => '', 'btwnr' => ''
        ),
        'delivery'       => array(
          'firstname' => '',  'lastname' => '', 'company' => '',  'street_address' => '',
          'suburb' => '',     'city' => '',     'postcode' => '', 'state' => '',
          'zone_id' => '',    'country' => array('id' => '', 'title' => '', 'iso_code_2' => '', 'iso_code_3' => ''),
          'country_id' => '', 'format_id' => '', 'btwnr' => ''
        ),
        'billing'        => array(
          'firstname' => '',  'lastname' => '', 'company' => '',  'street_address' => '',
          'suburb' => '',     'city' => '',     'postcode' => '', 'state' => '',
        ...
        if (is_object($$module) || $shipping == 'free_free') {
          ...
          if (isset($quote['error'])) {
            ...
            if (isset($quote[0]['methods'][0]['title']) && isset($quote[0]['methods'][0]['cost']) || $shipping == 'free_free') {
              ...
              'description' => (($shipping == 'free_free') ?  FREE_SHIPPING_TITLE : $quote[0]['module'] . ':<br />' . $quote[0]['methods'][0]['title']),
  ...
  function setCheckoutAddress($action){
    ...
    $tva_query = tep_db_query('select ab.billing_tva_intracom from customers c, ' . TABLE_ADDRESS_BOOK . ' ab where c.customers_id = "' . (int)$customer_id . '" and ab.customers_id = "' . (int)$customer_id . '" and c.customers_default_address_id = ab.address_book_id');
    $tva = tep_db_fetch_array($tva_query);
    ...
    if ($action == 'setBillTo'){
      ...
      if (tep_not_null($_POST['billing_email_address'])){
		...
		$order->{$varName}['email_address'] = $order->customer['email_address'];
      }
      if (tep_not_null($_POST['billing_telephone'])){
		...
		$order->{$varName}['telephone'] = $order->customer['telephone'];
      }
    ...
    $order->{$varName}['billing_tva_intracom'] = $tva['billing_tva_intracom'];
  ...
  function setAddress($addressType, $addressID){
    ...
    $Qaddress = tep_db_query('select ab.entry_firstname, ab.entry_lastname, ab.entry_company, ab.entry_street_address, ab.entry_suburb, ab.entry_postcode, ab.entry_city, ab.entry_zone_id, z.zone_name, ab.entry_country_id, c.countries_id, c.countries_name, c.countries_iso_code_2, c.countries_iso_code_3, c.address_format_id, ab.entry_state, ab.billing_tva_intracom from ' . TABLE_ADDRESS_BOOK . ' ab left join ' . TABLE_ZONES . ' z on (ab.entry_zone_id = z.zone_id) left join ' . TABLE_COUNTRIES . ' c on (ab.entry_country_id = c.countries_id) where ab.customers_id = "' . (int)$customer_id . '" and ab.address_book_id = "' . (int)$addressID . '"');
    ...
    $onepage[$sessVar] = array_merge($onepage[$sessVar], array(
      ...
      'country_id' => $address['entry_country_id'], 'format_id' => $address['address_format_id'], 'btwnr' => $address['billing_tva_intracom'], 'email_address' => $billto['email_address']
    ));
  ...
  function processCheckout(){
    ...
    */
    $html .= '<form name="redirectForm" action="' . $formUrl . '" method="POST">
		   ...
		   <input type="submit" value="'.IMAGE_CONTINUE.'" class="button-a" style="display:none;" />'.$hiddenFields.
		   '<script>
...
function createCustomerAccount(){
	...
	if ($onepage['createAccount'] === true && $this->checkEmailAddress($onepage['customer']['email_address'])){
		...
		$data = array(
			...
			'telephone' => $onepage['customer']['telephone'],
			'fax' => $onepage['customer']['fax'],
			'password' => $onepage['customer']['password'],
			'confirmation' => $onepage['customer']['password'],
			'street_address' => $onepage['billing']['street_address'],
			'postcode' => $onepage['billing']['postcode'],
			'city' => $onepage['billing']['city'],
			'country' => $onepage['billing']['country_id'],
			'forum_username' => '',
			'TermsAgree' => '1'
		);
		if (ACCOUNT_GENDER == 'true') $data['gender'] = $onepage['billing']['gender'];
		if (ACCOUNT_DOB == 'true') $data['dob'] = tep_date_raw($onepage['customer']['dob']);
		if (ACCOUNT_COMPANY == 'true') $data['company'] = $onepage['billing']['company'];
		if (ACCOUNT_COMPANY == 'true') $data['btwnr'] = $onepage['billing']['btwnr'];
		if (ACCOUNT_SUBURB == 'true') $data['entry_suburb'] = $onepage['billing']['suburb'];
		if (ACCOUNT_STATE == 'true') {
