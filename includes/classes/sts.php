...
class sts {
  var $sts_block, $template,
	...
	function stop_capture($block_name = '', $action = '')
	{
		...
		if (!$this->display_template_output) {
			return;
		} // Do not process anything if we are not in using templates
		$block = ob_get_contents(); // Get content of buffer
		ob_end_clean(); // Clear out the capture buffer
		if ($block_name == '') {
			return $block;
		} // Not need to continue if we don't want to save the buffer
		switch ($action) {
			case 'box':
	...
	function replace()
	{
		...
		$this->template['template_html'] = str_replace('<!--$headcontent-->', $this->template['headcontent'],
			$this->template['template_html']);
		// Manually replace the <!--$extracss--> with template['extracss']
		$this->template['template_html'] = str_replace('<!--$extracss-->', $this->template['extracss'],
			$this->template['template_html']);
	...
	function image(&$src)
	{
		...
		if (MODULE_STS_DEFAULT_STATUS == "true" && $this->is_button == false) {
			if (file_exists(STS_TEMPLATE_DIR . $src)) {
				$src = STS_TEMPLATE_DIR . $src;
			}
		}
