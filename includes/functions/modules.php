		...
		if (strstr($text, '[box]')) {
			...
			foreach ($matches[0] as $modulekey => $modulevalue) {
				...
				if ($matches[1][$modulekey] == 'compare.php') {
					if ((PRODUCT_COMPARE == 'true') && (($_GET['cPath']) || (strstr($_SERVER['PHP_SELF'],
					...
					$content = '<div class="box ' . str_replace('.php', '',
							$matches[1][$modulekey]) . '">' . $thisblockcontent . '</div>';
				}
...
function loadBox($boxName, $templateDir = 'mobile')
{
	$f = DIR_FS_CATALOG . 'templates/' . $templateDir . '/boxes/' . $boxName . '.php';
	if (file_exists($f)) {
		include $f;
		return true;
	}
