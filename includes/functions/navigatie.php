...
function abo_get_navigation($parent_id = 0, $level = 0) {
    global $languages_id;
    ...
    while ($nav_element = tep_db_fetch_array($resource)) {
        ...
        if (abo_has_subnavigation($nav_element['id'])) {
            ...
            $output .= abo_get_navigation($nav_element['id'], ($level + 1));
            $output .= '</ul>';
        }
...
function getCategoryList($parent_id, $level) {
	...
	while ($categories = tep_db_fetch_array($cat_query)) {
		...
		/*language fallback*/
		$category_list .= '<li><a href="'.tep_href_link(FILENAME_DEFAULT, 'cPath='.$categories['categories_id']).'" title="'.$categories['categories_name'].' - '.STORE_NAME.'">'.$categories['categories_name'].'</a>';
...
function getStyledCategoryList($parent_id = 0, $level = 0) {
    global $languages_id;
