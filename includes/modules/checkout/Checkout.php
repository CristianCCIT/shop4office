...
class Checkout extends Modules
{
	...
	public static $checkout_steps = array(
		...
		4 => array(
			'title' => 'Betaalmethode',
			'modules' => array(1 => 'coupon', 2 => 'payment', 3 => 'cadeaubon', 4 => 'extra')
		),
		'99' => array(
			'title' => 'Overzicht',
			'modules' => array(
	...
	public function __construct($show_step = true)
	{
		...
		if (isset($_GET['install'])) {
			...
			parent::addCron($min = '0', $hour = '4', $dayOfMonth = '1', $month = '*', $dayOfWeek = '*',
				$cmd = '/usr/bin/php -q ' . dirname(__FILE__) . '/assets/cron/cron.php');
		}
...
#		}
		...
		if (isset($_GET['force_checkout_step'])) {
			...
			if (isset($_GET['checkout_step']) && $temp_orders_id > 0 && $last_step >= $_GET['checkout_step']) {
				$step = $_GET['checkout_step'];
				//check if the right step is requested
			} else {
				if ($temp_orders_id > 0 && (count($_POST) > 0)) {
					if ($getstep == '') {
	...
	public function next_active_step($step)
	{
		...
		if ($step == '') {
			...
			while (key(self::$checkout_steps) != $step) {
	...
	public function prev_active_step($step)
	{
		...
		reset(self::$checkout_steps);
		//set pointer of array to selected step
		while (key(self::$checkout_steps) != $prev_step) {
			next(self::$checkout_steps);
		}
		next(self::$checkout_steps);
	...
	public function last_active_step()
	{
		...
		while (self::check_step(key($steps)) === false) {
			prev($steps);
		}
		return key($steps);
	}
		...
		foreach (self::$checkout_steps as $step => $stepdata) {
			if ($this->check_step($step)) {
				...
				if ($current_step == $step) {
					...
					if ($current_step > $step) {
						$html .= '<div class="step step' . $step . ' active">';
						$html .= '<div class="step_title"><a href="' . tep_href_link(basename($_SERVER['PHP_SELF']),
								'checkout_step=' . $step) . '" title="' . Translate($stepdata['title']) . '">' . $count . '. ' . Translate($stepdata['title']) . '</a></div>';
						$html .= '</div>';
					} else {
...
//////////////////////////////////////////////////////////////////////////// 07-06-2013
				if (STOCK_LIMITED == 'true') {
					...
					$this->setQuantityInProductsTable($orders_products[$products_order_id]['products_id'],
						$orders_products[$products_order_id]['products_quantity']);
				}
//////////////////////////////////////////////////////////////////////////// 07-06-2013
				...
				if (is_array($orders_products_attributes)) {
					foreach ($orders_products_attributes[$products_order_id] as $key => $value) {
						...
						tep_db_perform('orders_products_attributes',
							$orders_products_attributes[$products_order_id][$key], 'insert');
					}
				}
			}
		}
	...
	public function send_order_mail($order_id)
	{
		...
		if (!empty($this->temp_data[$order_id]['orders']['delivery_company'])) {
			$shipping_address['company'] = $this->temp_data[$order_id]['orders']['delivery_company'];
		} else {
			$shipping_address['company'] = $this->temp_data[$order_id]['orders']['billing_company'];
		}

		if (!empty($shipping_address['tva_intracom'])) {
			$shipping_address['tva_intracom'] = $this->temp_data[$order_id]['orders']['delivery_tva_intracom'];
		} else {
			$shipping_address['tva_intracom'] = $this->temp_data[$order_id]['orders']['billing_tva_intracom'];
		}

		$Varshipaddress = tep_address_format($this->temp_data[$order_id]['orders']['delivery_address_format_id'],
			$shipping_address, $html = false, $boln = '', $eoln = "\n");
		...
		$Varadpay = tep_address_format($this->temp_data[$order_id]['orders']['billing_address_format_id'],
			$payment_address, $html = false, $boln = '', $eoln = "\n");
		...
		//send email
		if (tep_mail($this->temp_data[$order_id]['orders']['customers_name'],
			$this->temp_data[$order_id]['orders']['customers_email_address'], Translate('Verwerking bestelling'),
			$email_order, STORE_NAME, STORE_OWNER_EMAIL_ADDRESS)) {
			tep_db_query('UPDATE orders_status_history SET customer_notified = 1 WHERE orders_id = "' . $order_id . '"');
		}

		//send extra emails
		if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
			tep_mail('', SEND_EXTRA_ORDER_EMAILS_TO, Translate('Verwerking bestelling'), $email_order, STORE_NAME,
...
////////////////////////////////////////////////////////// 21-05-2013 //customer email on order status
	...
	public function send_order_error_mail($subject, $content, $customerEmail = '')
	{
		...
		tep_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, $tempSubject, $email_order, STORE_NAME,
			STORE_OWNER_EMAIL_ADDRESS);
