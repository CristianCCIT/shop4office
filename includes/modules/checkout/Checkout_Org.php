...
class Checkout extends Modules {
				...
				if ($current_step == $step) {
					...
					if (count(array_intersect_key(parent::$disable_next_button, $_GET)) == 0) {
						if ($this->last_active_step() == $current_step) {
							$html .= '<button type="submit" class="btn btn-large btn-cta">'.Translate('Bestelling bevestigen').'</button>';
						} else {
	...
	public function send_order_error_mail($subject, $content) {
		$html_products = '';
		...
		/*SEND EMAIL*/
		tep_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, $subject, $email_order, Translate('Bestellingen'), STORE_OWNER_EMAIL_ADDRESS);
	}	
