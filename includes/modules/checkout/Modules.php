...
class Modules
{
	...
	public function get_country_list($name, $selected = '', $parameters = '')
	{
		...
		$query = tep_db_query('SELECT countries_name, countries_id FROM countries WHERE countries_id IN ("' . implode('", "',
				self::$available_countries) . '")');
		while ($array = tep_db_fetch_array($query)) {
			$countries_array[] = array('id' => $array['countries_id'], 'text' => Translate($array['countries_name']));
		}
	...
	public function checkTranslations($directory, $translations)
	{
		...
		foreach ($translations as $string => $translationData) {
			foreach ($translationData as $id => $translation) {
				if (!$this->translationExists($id, $string)
				) {//Check if this string exists in translation table for this language
		...
		foreach ($files as $file) {
			...
			foreach ($matches[1] as $string) {//loop through all translation strings
				foreach ($languages as $lngData) {//loop through all active languages
					if (!$this->translationExists($lngData['id'], $string)
					) {//Check if this string exists in translation table
						//We don't have a translation
	...
	public function generateCSSFile()
	{
		...
		$minifyCCS->minify(dirname(__FILE__) . '/cache/stylesheet.php', $stripComments = true, $stripWhitespace = true,
			$shortenHex = true, $combineImports = true, $importFiles = true);
		/*}*/
	...
	public function generateJSFile()
	{
		if (!is_file(dirname(__FILE__) . '/cache/javascript.php')) {
			...
			if (strlen($js) > 0) {
				...
				$minifyJS->minify(dirname(__FILE__) . '/cache/javascript.php', $stripComments = true,
