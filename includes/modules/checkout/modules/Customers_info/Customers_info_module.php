	...
	public function __construct()
	{
		...
		while ($array = tep_db_fetch_array($query)) {
			if ($array['type'] == 'config') {
				$this->config[$array['name']] = array(
					'value' => $array['value'],
					'options' => unserialize($array['options'])
				);
			} else {
				if (($array['type'] == 'field' || $array['type'] == 'dropdown') && $array['value'] == 'true') {
			...
			if (isset($_GET['add_new_address']) && $_GET['add_new_address'] == 'true' && $this->config['change_billing_address']['value'] == 'true') {
							...
							if ($show_field) {
								...
								if ($field_data['type'] == 'dropdown') {
									if (strstr($field_data['dbvalue'], 'country')) {
										$html .= parent::get_country_list($field_data['dbvalue'],
											$_POST[$field_data['dbvalue']],
											'id="address_book_item_' . $field_data['dbvalue'] . '"');
									}
								} else {
				...
				foreach ($this->fields as $block => $block_data) {
					...
					if ($block == 'billing' && $this->config['change_billing_address']['value'] == 'false') {
						$ab_query = tep_db_query('SELECT * FROM address_book WHERE customers_id = "' . $customer_id . '" AND entry_country_id IN ("' . implode('", "',
								parent::$available_countries) . '") AND address_book_id = "' . $c['customers_default_address_id'] . '"');
					} else {
						$ab_query = tep_db_query('SELECT * FROM address_book WHERE customers_id = "' . $customer_id . '" AND entry_country_id IN ("' . implode('", "',
								parent::$available_countries) . '")');
					}
					while ($ab = tep_db_fetch_array($ab_query)) {
						...
						if ($c['customers_default_address_id'] == $ab['address_book_id']) {
							...
							$html .= '<div class="delete_address"><a href="' . tep_href_link(basename($_SERVER['PHP_SELF']),
									'delete_address=' . $ab['address_book_id']) . '" title="' . Translate('Verwijder adres') . '" class="btn btn-mini">' . Translate('Verwijder adres') . '</a></div>';
						}
					...
					if (($block == 'billing' && $this->config['change_billing_address']['value'] == 'true') || $block != 'billing') {
						$html .= '<div class="add_new_address"><a href="' . tep_href_link(basename($_SERVER['PHP_SELF']),
								'checkout_step=' . $step . '&add_new_address=true') . '" title="' . Translate('Nieuw adres toevoegen') . '" class="btn btn-mini"><i class="icon-plus-sign"></i> ' . Translate('Nieuw adres toevoegen') . '</a></div>';
					}
					$html .= '</div>';//end well
					$html .= '</div>';//end span6
				}
					...
					if ($show_field) {
						...
						if ($field_data['type'] == 'dropdown') {
							if (strstr($field, 'country')) {
								$html .= parent::get_country_list(get_class($this) . '_' . $field,
									(isset($_POST[get_class($this) . '_' . $field]) ? $_POST[get_class($this) . '_' . $field] : $to[$field]),
									'id="' . get_class($this) . '_input_' . $field . '"');
							}
						} else {
							if ($field_data['name'] == 'billing_firstname') {
								...
								if ($field_data['name'] == 'billing_lastname') {
									if (isset($_POST[get_class($this) . '_' . $field])) {
										...
										$billing_lastname = substr($billing_lastname, 0,
											-(strlen($billing_lastname_temp)));
									}
									...
									if ($field_data['name'] == 'delivery_firstname') {
										...
										if ($field_data['name'] == 'delivery_lastname') {
											if (isset($_POST[get_class($this) . '_' . $field])) {
												...
												$delivery_lastname = substr($delivery_lastname, 0,
													-(strlen($delivery_lastname_temp)));
											}
...
////////////////////////////////////////////////////////////////* #1539 - Shipping address
			...
			if ($this->config['create_account']['value'] == 'true' && !tep_session_is_registered('customer_id')) {
				...
				$termsAgree = sprintf(Translate("Ik heb de %s gelezen en ga hiermee akkoord"),
					'<a href="' . tep_href_link('conditions_modal.php') . '" target="_blank">' . Translate('Algemene voorwaarden') . '</a>');
							...
							foreach ($this->fields[$block]['fields'] as $field => $fieldData) {
								...
								if (isset($ab[$fieldData['dbvalue']])) {
									...
									if ($fieldData['expression'] != '') {
										...
										if (!preg_match($fieldData['expression'],
											convert_to_entities($ab[$fieldData['dbvalue']]))
										) {
					...
					foreach ($blockData['fields'] as $field => $fieldData) {
						...
						if ($fieldData['block'] == 'delivery' && $billing_is_delivery) {
							...
							if (isset($_POST[get_class($this) . '_' . $field])) {
								...
								if ($billing_is_delivery && strstr($field,
										'billing') && $field != 'billing_tva_intracom'
								) {
					...
					if ($_POST[get_class($this) . '_password'] == $_POST[get_class($this) . '_password2']) {
						...
						if (isset($create_customer['errors'])) {
							...
							if (isset($_POST['different_delivery_address']) && $_POST['different_delivery_address'] == 'true') {
								...
								$address_book_item = $Customer->create_address_book_item($customer_id,
									$address_book_data);
	...
	public function administrator()
	{
		...
		?>
		<form name="<?php echo get_class($this); ?>" class="form-horizontal"
			...
			<table class="table table-striped table-bordered">
				...
				while ($db = tep_db_fetch_array($db_query)) {
					...
					<tr<?php echo($db['value'] == 'false' ? ' class="inactive"' : ''); ?>>
						<td><input type="text" name="name[<?php echo $db['id']; ?>]" value="<?php echo $db['name']; ?>"
						           class="input-large"/></td>
						<td>
							<select name="type[<?php echo $db['id']; ?>]" class="input-medium">
								<option
									value="config"<?php echo($db['type'] == 'config' ? ' selected="selected"' : ''); ?>><?php echo Translate('configuratie'); ?></option>
								<option
									value="field"<?php echo($db['type'] == 'field' ? ' selected="selected"' : ''); ?>><?php echo Translate('Veld'); ?></option>
								<option
									value="dropdown"<?php echo($db['type'] == 'dropdown' ? ' selected="selected"' : ''); ?>><?php echo Translate('Dropdown'); ?></option>
							</select>
						</td>
						<td>
							<select name="input[<?php echo $db['id']; ?>]" class="input-medium">
								...
								<option
									value="text"<?php echo($db['input'] == 'text' ? ' selected="selected"' : ''); ?>><?php echo Translate('Tekst'); ?></option>
								<option
									value="email"<?php echo($db['input'] == 'email' ? ' selected="selected"' : ''); ?>><?php echo Translate('E-mail'); ?></option>
								<option
									value="tel"<?php echo($db['input'] == 'tel' ? ' selected="selected"' : ''); ?>><?php echo Translate('Telefoon'); ?></option>
							</select>
						</td>
						<td>
							<select name="value[<?php echo $db['id']; ?>]" class="input-small">
								<option
									value="true"<?php echo($db['value'] == 'true' ? ' selected="selected"' : ''); ?>><?php echo Translate('Actief'); ?></option>
								<option
									value="false"<?php echo($db['value'] == 'false' ? ' selected="selected"' : ''); ?>><?php echo Translate('Niet Actief'); ?></option>
							</select>
						</td>
						<td>
							<select name="block[<?php echo $db['id']; ?>]" class="input-medium">
								<option value=""><?php echo Translate('Niet van toepassing'); ?></option>
								<option
									value="delivery"<?php echo($db['block'] == 'delivery' ? ' selected="selected"' : ''); ?>><?php echo Translate('Leveringsadres'); ?></option>
								<option
									value="billing"<?php echo($db['block'] == 'billing' ? ' selected="selected"' : ''); ?>><?php echo Translate('Facturatieadres'); ?></option>
							</select>
						</td>
						<td><input type="text" name="label[<?php echo $db['id']; ?>]"
						           value="<?php echo $db['label']; ?>" class="input-medium"/></td>
						<td>
							<select name="dbvalue[<?php echo $db['id']; ?>]" class="input-large">
								...
								foreach ($dbvalues_array as $dbvalues) {
									?>
									<option
										value="<?php echo $dbvalues; ?>"<?php echo($db['dbvalue'] == $dbvalues ? ' selected="selected"' : ''); ?>><?php echo $dbvalues; ?></option>
								<?php
								}
								?>
							</select>
						</td>
						<td><input type="text" name="expression[<?php echo $db['id']; ?>]"
						           value="<?php echo stripslashes($db['expression']); ?>" class="input-large"/></td>
						<td><input type="text" name="function[<?php echo $db['id']; ?>]"
						           value="<?php echo stripslashes($db['function']); ?>" class="input-medium"/></td>
						<td><input type="text" name="condition[<?php echo $db['id']; ?>]"
						           value="<?php echo stripslashes($db['condition']); ?>" class="input-xlarge"/></td>
						<td><input type="text" name="error[<?php echo $db['id']; ?>]"
						           value="<?php echo stripslashes($db['error']); ?>" class="input-xlarge"/></td>
						<td><input type="number" name="sort_order[<?php echo $db['id']; ?>]"
						           value="<?php echo stripslashes($db['sort_order']); ?>" class="input-mini"/></td>
						<td><a href="<?php echo tep_href_link('checkout.php',
								'module=' . $_GET['module'] . '&action=delete&id=' . $db['id']); ?>"
						       class="btn btn-danger"><?php echo Translate('Verwijderen'); ?></a></td>
					</tr>
				<?php
				}
				?>
		...
		<div id="new_instance" style="display:none;">
			<form name="<?php echo get_class($this); ?>" class="form-horizontal well"
			      action="<?php echo tep_href_link('checkout.php', 'module=' . $_GET['module']); ?>" method="post">
