<?php
class Delivery_address extends Modules {
	public $type = 'delivery_address'
		 , $config = array();
	...
	public function output($step = 0) {
		...
		if ($temp_orders_id > 0) {
			...
			if (tep_db_num_rows($to_query) > 0) {
				...
				$html .= '<div class="delivery_address">';
				$html .= '<h3 class="delivery_address_title">'.Translate($this->config['title']).'</h3>';
				if (!empty($this->config['description'])) {
					$html .= '<p class="delivery_address_description">'.Translate($this->config['description']).'</p>';
				}
				$html .= '<blockquote>';
				$html .= '<address>';
				//name
				$html .= '<strong>'.$to['delivery_name'].'</strong><br />';

				//company
				if (!empty($to['delivery_company'])) $html .= $to['delivery_company'].'<br />';
				elseif (!empty($to['billing_company'])) $html .= $to['billing_company'].'<br />';
				if (!empty($to['delivery_tva_intracom'])) $html .= $to['delivery_tva_intracom'].'<br />';
				elseif (!empty($to['billing_tva_intracom'])) $html .= $to['billing_tva_intracom'].'<br />';
				//street
				$html .= $to['delivery_street_address'].'<br />';
				//city + postcode + country
				$html .= $to['delivery_postcode'].' '.$to['delivery_city'].' '.tep_get_country_name($to['delivery_country']).'<br />';
				$html .= '</address>';
				$html .= '</blockquote>';
				$html .= '</div>';//end delivery_address
			}
		}
		return $html;
	}
