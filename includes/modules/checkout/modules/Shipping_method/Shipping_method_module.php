...
class Shipping_method extends Modules
{
	...
	public function output($step = 0)
	{
		...
		if ($temp_orders_id > 0) {
			$sm_query = tep_db_query('SELECT shipping_method FROM temp_orders WHERE orders_id = "' . $temp_orders_id . '"');
			if (tep_db_num_rows($sm_query) > 0) {
				$sm = tep_db_fetch_array($sm_query);
				if (strstr($sm['shipping_method'], '_')) {
					$module_instance = end(explode('_', $sm['shipping_method']));
					$module = substr($sm['shipping_method'], 0, -(strlen($module_instance) + 1));
				} else {
					$module = $sm['shipping_method'];
				}
				global $$module;
				$html .= '<div class="shipping_method">';
				$html .= '<h3 class="shipping_method_title">' . Translate($this->config['title']) . '</h3>';
				if (!empty($this->config['description'])) {
					$html .= '<p class="shipping_method_description">' . Translate($this->config['description']) . '</p>';
				}
				$html .= '<blockquote>';
				if (isset($$module->instances)) {
					//method title
					$html .= '<p>' . Translate($$module->instances[$sm['shipping_method']]['title']);
					//method description
					if (isset($$module->instances[$sm['shipping_method']]['description'])) {
						$constants = get_defined_constants();
						$description = '';
						$description_lines = explode("\n",
							stripslashes($$module->instances[$sm['shipping_method']]['description']));
					...
					if (method_exists($$module, 'getTitle')) {
						$html .= '<p>' . Translate($$module->getTitle());
					} else {
						$html .= '<p>' . Translate($$module->config['title']);
					}
					//method description
					if (isset($$module->instances[$sm['shipping_method']]['description'])) {
						$constants = get_defined_constants();
		...
		<form name="<?php echo get_class($this); ?>" class="form-horizontal well"
			...
			<fieldset>
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" name="title" value="<?php echo $this->config['title']; ?>"
					...
					<div class="controls">
						<div class="btn-group" data-toggle="buttons-radio">
							<button type="submit" name="status" value="true"
							        class="btn<?php echo($this->config['status'] == 'true' ? ' active' : ''); ?>"><?php echo Translate('Actief'); ?></button>
							<button type="submit" name="status" value="false"
							        class="btn<?php echo($this->config['status'] == 'false' ? ' active' : ''); ?>"><?php echo Translate('Niet actief'); ?></button>
						</div>
					</div>
				</div>
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" name="description" value="<?php echo $this->config['description']; ?>"
