...
class Table extends Modules
{
		...
		foreach ($this->instances as $id => $data) {
			if ($data['status'] == 'true') {
				if (parent::checkZone($data['zone'], $selected['delivery_country'])) {
					...
					$html .= '<div class="' . $this->type . '_quote">' . ($this->calculate_quote($id,
							$this->instance_max[$id]) > 0 ? $currencies->display_price($this->calculate_quote($id,
							$this->instance_max[$id]), 0) : Translate('Gratis')) . '</div>';
	...
	public function process_data()
	{
		...
		if (isset($_POST[$this->type])) {
			if (substr($_POST[$this->type], 0, $strlen) == get_class($this)) {
				...
				if (isset($_POST['gift_wrap']) && $_POST['gift_wrap'] == "on") {
					$gift_wrap = "1";
				} else {
					$gift_wrap = "0";
				}
				...
				if (tep_db_num_rows($ot_query) > 0) {
					tep_db_query('UPDATE temp_orders_total SET title="' . Translate($this->instances[$_POST[$this->type]]['title']) . '", text="' . $currencies->display_price($this->calculate_quote($_POST[$this->type],
							$this->instance_max[$_POST[$this->type]]),
							0) . '", value="' . $this->calculate_quote($_POST[$this->type],
							$this->instance_max[$_POST[$this->type]]) . '", sort_order="' . $this->instances[$_POST[$this->type]]['sort_order'] . '" WHERE orders_id = "' . $temp_orders_id . '" AND class="' . $this->type . '"');
				} else {
					tep_db_query('INSERT INTO temp_orders_total (orders_id, title, text, value, class, sort_order) VALUES ("' . $temp_orders_id . '", "' . Translate($this->instances[$_POST[$this->type]]['title']) . '", "' . $currencies->display_price($this->calculate_quote($_POST[$this->type],
							$this->instance_max[$_POST[$this->type]]),
							0) . '", "' . $this->calculate_quote($_POST[$this->type],
							$this->instance_max[$_POST[$this->type]]) . '", "' . $this->type . '", "' . $this->instances[$_POST[$this->type]]['sort_order'] . '")');
				}
			...
			if ($data['status'] == 'true') {
				if (parent::checkZone($data['zone'], $country)) {
					...
					$array[] = array(
						...
						'quote' => $this->calculate_quote($id, $this->instance_max[$id])
					);
				}
			}
		}
	...
	public function administrator()
	{
		...
		echo get_class($this);
		if ($login == 'aboservice') {
			echo '<button type="button" id="delete_module" href="' . tep_href_link('checkout.php',
					'module=' . $_GET['module']) . '&action=delete_module" class="btn btn-danger pull-right">' . Translate('Verwijder module') . '</button>';
		...
		foreach ($this->instances as $instance => $instance_data) {
			?>
			<form name="<?php echo $instance; ?>" class="form-horizontal well"
			      action="<?php echo tep_href_link('checkout.php', 'module=' . $_GET['module']); ?>" method="post">
				<input type="hidden" name="action" value="save"/>
				<fieldset>
					<legend>
						<?php echo $instance; ?>
						<button class="btn btn-danger pull-right" type="submit" name="delete"
						        value="<?php echo $instance_data['id']; ?>"><?php echo Translate('Verwijderen'); ?></button>
					</legend>
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo $instance; ?>_title"><?php echo Translate('Titel'); ?></label>

						<div class="controls">
							<input type="text" name="title[<?php echo $instance_data['id']; ?>]"
							       value="<?php echo $instance_data['title']; ?>" class="input-xlarge"
							       id="<?php echo $instance; ?>_title"/>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo $instance; ?>_status"><?php echo Translate('Status'); ?></label>

						<div class="controls">
							<div class="btn-group" data-toggle="buttons-radio">
								<button type="submit" name="status[<?php echo $instance_data['id']; ?>]" value="true"
								        class="btn<?php echo($instance_data['status'] == 'true' ? ' active' : ''); ?>"><?php echo Translate('Actief'); ?></button>
								<button type="submit" name="status[<?php echo $instance_data['id']; ?>]" value="false"
								        class="btn<?php echo($instance_data['status'] == 'false' ? ' active' : ''); ?>"><?php echo Translate('Niet actief'); ?></button>
							</div>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo $instance; ?>_description"><?php echo Translate('Omschrijving'); ?></label>

						<div class="controls">
							<input type="text" name="description[<?php echo $instance_data['id']; ?>]"
							       value="<?php echo $instance_data['description']; ?>" class="input-xxlarge"
							       id="<?php echo $instance; ?>_description"/>
							<span
								class="help-block"><?php echo Translate('Omschrijving gebruikt in de checkout, samen met de titel'); ?></span>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo $instance; ?>_order_status_id"><?php echo Translate('Bestel status'); ?></label>

						<div class="controls">
							<select name="order_status_id[<?php echo $instance_data['id']; ?>]" class="input-xlarge"
							        id="<?php echo $instance; ?>_order_status_id">
								<?php
							...
							<span
								class="help-block"><?php echo Translate('De status na het plaatsen van de bestelling met deze methode.'); ?></span>
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<label class="checkbox inline"><?php echo Translate('All'); ?>
								<input type="checkbox" name="zone[<?php echo $instance_data['id']; ?>][*]"
								       value="*" <?php echo($instance_data['zone'] == '*' ? ' checked="checked"' : ''); ?> />
							</label>
							<?php
							...
							foreach ($zones as $zone_id => $zone_name) {
								...
								echo '<input type="checkbox" name="zone[' . $instance_data['id'] . '][' . $zone_id . ']" value="' . $zone_id . '"' . (in_array($zone_id,
										$selected_zones) ? ' checked="checked"' : '') . ' />';
								echo '</label>';
							}
							?>
							<span
								class="help-block"><?php echo Translate('Voor welke zone is deze methode actief'); ?></span>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo $instance; ?>_mode"><?php echo Translate('Mode'); ?></label>

						<div class="controls">
							<div class="btn-group" data-toggle="buttons-radio">
								<button type="submit" name="mode[<?php echo $instance_data['id']; ?>]" value="price"
								        class="btn<?php echo($instance_data['mode'] == 'price' ? ' active' : ''); ?>"><?php echo Translate('Prijs'); ?></button>
								<button type="submit" name="mode[<?php echo $instance_data['id']; ?>]" value="weight"
								        class="btn<?php echo($instance_data['mode'] == 'weight' ? ' active' : ''); ?>"><?php echo Translate('Gewicht'); ?></button>
							</div>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo $instance; ?>_quote"><?php echo Translate('Verzendkost'); ?></label>

						<div class="controls">
							<input type="text" name="quote[<?php echo $instance_data['id']; ?>]"
							       value="<?php echo $instance_data['quote']; ?>" class="input-xlarge"
							       id="<?php echo $instance; ?>_quote"/>
							<span
								class="help-block"><?php echo Translate('De verzendkost gebasseerd op de totale prijs of gewicht van de artikels. VB. 25:8.50;50:5.50;... Tot 25 is de kostprijs 8.50, vanaf 25 tot 50 is de kostprijs 5.50, ...'); ?></span>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo $instance; ?>_sort_order"><?php echo Translate('Volgorde'); ?></label>

						<div class="controls">
							<input type="text" name="sort_order[<?php echo $instance_data['id']; ?>]"
							       value="<?php echo $instance_data['sort_order']; ?>" class="input-xlarge"
							       id="<?php echo $instance; ?>_sort_order"/>
							<span
								class="help-block"><?php echo Translate('Volgorde waarin de instanties worden getoond in de checkout.'); ?></span>
						</div>
					</div>
		...
		<div id="new_instance" style="display:none;">
			<form name="<?php echo get_class($this) . '_' . $new_id; ?>" class="form-horizontal well"
			      action="<?php echo tep_href_link('checkout.php', 'module=' . $_GET['module']); ?>" method="post">
				<input type="hidden" name="action" value="save"/>
				<fieldset>
					<legend>
					...
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo get_class($this) . '_' . $new_id; ?>_title"><?php echo Translate('Titel'); ?></label>

						<div class="controls">
							<input type="text" name="title[<?php echo $new_id; ?>]" value="" class="input-xlarge"
							       id="<?php echo get_class($this) . '_' . $new_id; ?>_title"/>
						</div>
					</div>
					...
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo get_class($this) . '_' . $new_id; ?>_description"><?php echo Translate('Omschrijving'); ?></label>

						<div class="controls">
							<input type="text" name="description[<?php echo $new_id; ?>]" value="" class="input-xxlarge"
							       id="<?php echo get_class($this) . '_' . $new_id; ?>_description"/>
							<span
								class="help-block"><?php echo Translate('Omschrijving gebruikt in de checkout, samen met de titel'); ?></span>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo get_class($this) . '_' . $new_id; ?>_order_status_id"><?php echo Translate('Bestel status'); ?></label>

						<div class="controls">
							<select name="order_status_id[<?php echo $new_id; ?>]" class="input-xlarge"
							        id="<?php echo get_class($this) . '_' . $new_id; ?>_order_status_id">
								<?php
								$statusses = parent::get_order_statusses();
								foreach ($statusses as $id => $name) {
					...
					<div class="control-group">
						<label class="control-label"
						       for="<?php echo get_class($this) . '_' . $new_id; ?>_sort_order"><?php echo Translate('Volgorde'); ?></label>

						<div class="controls">
							<input type="text" name="sort_order[<?php echo $new_id; ?>]" value="" class="input-xlarge"
							       id="<?php echo get_class($this) . '_' . $new_id; ?>_sort_order"/>
							<span
								class="help-block"><?php echo Translate('Volgorde waarin de instanties worden getoond in de checkout.'); ?></span>
						</div>
					</div>
	...
	{
		return array(
			'Opgelet!' => array('1' => 'Opgelet!', '2' => 'Attention!', '3' => 'Attention!', '4' => 'Achtung!')
		,
		...
		,
			'Verzending België' => array(
				'1' => 'Verzending België',
				'2' => 'Livraison en Belgique',
				'3' => 'Shipping Belgium',
				'4' => 'Liefer-Belgien'
			)
		,
