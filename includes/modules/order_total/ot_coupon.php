...
function ot_coupon() {
  ...
  $this->code = 'ot_coupon';
  $this->header = Translate('Gift Vouchers &amp; Coupons');
  $this->title = Translate('Discount Coupons');
  $this->description = Translate('Discount Coupon');
  $this->user_prompt = '';
...
global $_SERVER, $order, $currencies;
  ...
  if ($od_amount > 0) {
    ...
    $this->output[] = array('title' => $this->title . ': ' . $this->coupon_code.$percent.':',
...
// get some info from the coupon table
  ...
  if ($coupon_result['coupon_type'] != 'G') {
    ...
    if (tep_db_num_rows($coupon_query)==0) {
      tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error=' . urlencode(Translate('Invalid Coupon Code')), 'SSL'));
    }
    ...
    if (tep_db_num_rows($date_query)==0) {
      tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error=' . urlencode(Translate('This coupon is not available yet')), 'SSL'));
    }
  ...
  if (tep_db_num_rows($date_query)==0) {
      tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error=' . urlencode(Translate('This coupon has expired')), 'SSL'));
    }
    ...
    if (tep_db_num_rows($coupon_count)>=$coupon_result['uses_per_coupon'] && $coupon_result['uses_per_coupon'] > 0) {
      tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error=' . urlencode(Translate('This coupon can only be used').' '. $coupon_result['uses_per_coupon'].' '.Translate('times')), 'SSL'));
    }

    if (tep_db_num_rows($coupon_count_customer)>=$coupon_result['uses_per_user'] && $coupon_result['uses_per_user'] > 0) {
      tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error=' . urlencode(Translate('You have used the coupon maximum number of times allowed per customer.') . $coupon_result['uses_per_user'] .' '. Translate('times') ), 'SSL'));
    }
...
//  $err_msg = ERROR_REDEEMED_AMOUNT.ERROR_REDEEMED_AMOUNT_ZERO;
    $err_msg = Translate('*** This is a valid coupon code. HOWEVER: No price reduction can be applied, please see the coupon restrictions that were sent with your email offer. **');
    } else {
...
//$cc_id = $_SESSION['cc_id']; //Fred commented out, do not use $_SESSION[] due to backward comp. Reference the global var instead.
  ...
  if (isset($cc_id) ) {
    ...
    if (tep_db_num_rows($coupon_query) !=0 ) {
	  ...
	  } else if ($get_result['coupon_type'] == 'P') {
		  $this->percent = number_format($c_deduct).'%';
      ...
      if ($get_result['coupon_minimum_order'] <= $this->get_order_total()) {
        if ($get_result['restrict_to_products'] || $get_result['restrict_to_categories']) {
		  ...
		  if ($get_result['coupon_type'] == 'P' && $get_result['restrict_to_products']) {
			  ...
			  $this->percent .= ')';
		  } else if (strlen($this->percent) > 2 && $get_result['restrict_to_products']) {
			  $this->percent = substr($this->percent,0 ,-2);
			  $this->percent .= ')';
		  }
        }
...
function get_error() {
  ...
  $error = array('title' => Translate('Coupon already redeemed'),
