...
*/
  ...
  class ot_gv {
    ...
    function ot_gv() {
      $this->code = 'ot_gv';
      $this->title = Translate('Cadeaubonnen');
      $this->header = Translate('Enter code below if you have any gift certificates or coupons you have not redeemed yet.');
      $this->description = Translate('Cadeaubonnen');
      $this->user_prompt = '';
...
// if there has been something before us then
 ...
 // #################### Begin Added CGV JONYO ######################
	function use_credit_amount() {
		global $cot_gv, $currencies, $GLOBALS;
//      $_SESSION['cot_gv'] = false;     // old code - Strider
		$cot_gv = false;
		if ($this->selection_test()) {
			if ($this->enabled && $this->output[0]['value'] > 0 ) {
				$output_string = str_replace('>', 'checked="checked">', $this->checkbox)."\n";
			} else {
...
//        $gv_order_amount += 0.001;
    ...
    function credit_selection() {
      ...
      if (tep_db_num_rows($gv_query) > 0) {
		  $selection_string .= '<table cellspacing="0" cellpadding="0" border="0" width="100%">';
		  $selection_string .= '<tr>';
		  $selection_string .= '<td>'.Translate('Vul uw cadeaubon code in').':&nbsp;';
		  $selection_string .= '<input type="text" name="gv_redeem_code" />&nbsp;';
		  $selection_string .= '<input type="button" class="button-a" onclick="submitFunction();" value="'.Translate('Pas cadeaubon code toe').'" /></td>';
		  $selection_string .= '</tr>';
...
// START added by Rigadin in v5.13, needed to show module errors on checkout_payment page
    function get_error() {
      ...
      $error = array('title' => Translate('Gift Voucher/Discount coupon'),
