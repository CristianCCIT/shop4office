...
class icepay_cc_mastercard extends icepay
{

	var $icon = "creditcard.jpg";

	function icepay_cc_mastercard()
	{
		global $order;

		$this->code				= 'icepay_cc_mastercard';
		...
		$this->sort_order		= MODULE_PAYMENT_ICEPAY_CC_MASTERCARD_SORT_ORDER;
		$this->enabled			= ((MODULE_PAYMENT_ICEPAY_CC_MASTERCARD_STATUS == 'True') ? true : false);
	...
	function getLangStr($str){
		switch ($str){
			case "title":
				return Translate('Betalen met Mastercard');
				break;
		}
	}
	...
	function process_button() {
        ...
        return 	tep_draw_hidden_field('ic_paymentmethod',		'CREDITCARD').
				tep_draw_hidden_field('ic_issuer',		'MASTER').
				$hidden_fields;
	}
	...
	function check()
	{
		...
		{
			$check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_ICEPAY_CC_MASTERCARD_STATUS'");
	...
	function install()
	{
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_ICEPAY_CC_MASTERCARD_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
	...
	function keys()
	{
		return array
		(
			'MODULE_PAYMENT_ICEPAY_CC_MASTERCARD_STATUS',
			'MODULE_PAYMENT_ICEPAY_CC_MASTERCARD_SORT_ORDER',
