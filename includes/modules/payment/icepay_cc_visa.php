...
class icepay_cc_visa extends icepay
{

	var $icon = "creditcard.jpg";

	function icepay_cc_visa()
	{
		global $order;

		$this->code				= 'icepay_cc_visa';
		...
		$this->sort_order		= MODULE_PAYMENT_ICEPAY_CC_VISA_SORT_ORDER;
		$this->enabled			= ((MODULE_PAYMENT_ICEPAY_CC_VISA_STATUS == 'True') ? true : false);
		...
		$this->form_action_url = $this->base_url . '?type=creditcard';
	}
	
	function getLangStr($str){
		switch ($str){
			case "title":
				return Translate('Betalen met Visa');
				break;
		}
	}
	
	function process_button() {
		$hidden_fields = parent::process_button(); 
        return 	tep_draw_hidden_field('ic_paymentmethod',		'CREDITCARD').
				tep_draw_hidden_field('ic_issuer',		'VISA').
				$hidden_fields;
	}
	...
	function check()
	{
		...
		{
			$check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_ICEPAY_CC_VISA_STATUS'");
	...
	function install()
	{
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable ICEPAY VISA Module', 'MODULE_PAYMENT_ICEPAY_CC_VISA_STATUS', 'True', 'Do you want to accept VISA payments?', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_ICEPAY_CC_VISA_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
	...
	function keys()
	{
		return array
		(
			'MODULE_PAYMENT_ICEPAY_CC_VISA_STATUS',
			'MODULE_PAYMENT_ICEPAY_CC_VISA_SORT_ORDER',
