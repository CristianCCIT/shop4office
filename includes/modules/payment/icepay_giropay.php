...
class icepay_giropay extends icepay
{
	...
	function icepay_giropay()
	{
		...
		$this->enabled			= ($this->doAllowedCurrencyCheck('EUR') && (MODULE_PAYMENT_ICEPAY_GIROPAY_STATUS == 'True' && $this->doAllowedCountryCheck($this->allowedCountriesGiropay())) ? true : false);
		...
		$this->form_action_url = $this->base_url . '?type=giropay';
	}
	
	function getLangStr($str){
		switch ($str){
			case "title":
				return Translate('Betaling via Giropay');
				break;
		}
	}
