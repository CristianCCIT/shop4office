...
class icepay_sms extends icepay
{
	...
	function icepay_sms()
	{
		...
		$this->enabled			= ((MODULE_PAYMENT_ICEPAY_SMS_STATUS == 'True') && $this->doAllowedCountryCheck($this->allowedCountriesSMS()) ? true : false);
		...
		$this->form_action_url = $this->base_url . '?type=sms';
	}
	
	function getLangStr($str){
		switch ($str){
			case "title":
				return Translate('Betaling via SMS');
				break;
		}
	}
