...
if (!tep_session_is_registered('products_per_search_page')) tep_session_register('products_per_search_page');
if (!isset($products_per_search_page) || !is_integer($products_per_search_page) || ($products_per_search_page < 1)) $products_per_search_page = MAX_DISPLAY_SEARCH_RESULTS;
if (isset($_GET['ppp']) && is_numeric($_GET['ppp']) && ($_GET['ppp'] > 0)) $products_per_search_page = intval($_GET['ppp']);
$ppp_list = array();
for ($i=1;$i<=10;$i++) {
...
if (strstr($_SERVER['PHP_SELF'], FILENAME_AJAX_SEARCH) || $specification_group != '') {
	$numprodform = tep_draw_form('prod_per_page', tep_href_link(basename($_SERVER['PHP_SELF']), tep_get_all_get_params(array('ppp', 'page'))), 'get').Translate('Aantal producten op een pagina').': '.tep_draw_pull_down_menu('ppp', $ppp_list, $products_per_search_page, 'class="smallText"') . '</form>';
} else {
	$numprodform = tep_draw_form('prod_per_page', tep_href_link(basename($_SERVER['PHP_SELF']), tep_get_all_get_params(array('ppp', 'page'))), 'get').Translate('Aantal producten op een pagina').': '.tep_draw_pull_down_menu('ppp', $ppp_list, $products_per_search_page, 'class="smallText" onchange="this.form.submit()"');
...
if ($listing_split->number_of_rows > 0) {
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="2">
			<tr>
				<td align="left" class="pages">
					<?php echo $listing_split->display_count(Translate('Product <b>%d</b> tot <b>%d</b> (van <b>%d</b> producten)')); ?><br />
					<?php echo Translate('Paginas') . ': ' . $listing_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?>
				</td>
				<td align="right">
					<?php
					...
					if (PRODUCT_LIST_PRINT=='true') {
						...
						?>
							<a href="#" id="print_all" class="<?php echo $current_category_id; ?>"><?php echo Translate('Catalogus afdrukken'); ?></a>
	...
	if ($total_columns > 0) {
		...
		$column_list=0;
		if ($display_class=='grid') {
			?><div class="column column-sortby"><?php echo Translate('Sorteren op'); ?></div><?php
		}
		while ($list_config = tep_db_fetch_array($list_config_query)) {
			if ($list_config['configuration_value']>0) {
				...
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_DESCRIPTION') {
					$lc_text = Translate('Omschrijving');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_TECHNICAL') {
					$lc_text = Translate('Technische info');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_MANUFACTURER') {
					$lc_text = Translate('Merk');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_QUANTITY') {
					$lc_text = Translate('Hoeveelheid');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_IMAGE') {
					$lc_text = Translate('Afbeelding');
					$column_width = $column_width/2;
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_WEIGHT') {
					$lc_text = Translate('Gewicht');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_PRICE') {
					$lc_text = Translate('Prijs');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_BUY_NOW') {
					$lc_text = Translate('Kopen');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT1') {
					$lc_text = Translate('Optieveld 1');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT2') {
					$lc_text = Translate('Optieveld 2');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT3') {
					$lc_text = Translate('Optieveld 3');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT4') {
					$lc_text = Translate('Optieveld 4');
				} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT5') {
					$lc_text = Translate('Optieveld 5');
				} else if (($list_config['configuration_key']=='PRODUCT_'.$listtype.'_COMPARE') && (PRODUCT_COMPARE=='true')) {
				...
				if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_SPECS') {
					for ($s=0 ; $s<PRODUCT_LIST_SPECS_COUNT; $s++) {
						if ($checkboxes[$s]['id']!='') {
							?>
							<div class="column column-<?php echo $column_list.$column_class; ?>" <?php if ($display_class=='list') { ?>style="width:<?php echo $column_width; ?>%;"<?php } ?>>
								...
								<?php
									if ((PRODUCT_LIST_HEADING=='dynamic') && ($display_class=='list')) {
											...
											echo tep_draw_pull_down_menu('PRODUCT_'.$listtype.'_SPECS_'.$s, $checkboxes, $default_selected, ' style="width:65%;"').tep_create_sort_heading($_GET['sort'], $column_list, '');
									} else {
										if (PRODUCT_LIST_HEADING!='static') {
											echo tep_create_sort_heading($_GET['sort'], $column_list, $checkboxes[$s]['text'], true);
										} else {
											?><span class="static"><?php echo $checkboxes[$s]['text']; ?></span><?php
										}
									}
								?>
								</div>
					...
					<div class="column column-<?php echo $column_list.$column_class; ?>" <?php if ($display_class=='list') { ?>style="width:<?php echo $column_width; ?>%;"<?php } ?>>
						...
						<?php 
							if (($list_config['configuration_key']!='PRODUCT_'.$listtype.'_IMAGE') && ($list_config['configuration_key']!='PRODUCT_'.$listtype.'_BUY_NOW') && ($list_config['configuration_key']!='PRODUCT_'.$listtype.'_COMPARE') && (PRODUCT_LIST_HEADING!='static') && ($listing_type!='specials_listing') && ($listing_type!='products_new_listing') && ($listing_type!='search_results')) {
								...
								?>
                                <span class="static"><a href="<?php echo tep_href_link(FILENAME_COMPARE_PRINT, 'cc=' . $compare_category); ?>" <?php if (PRODUCT_COMPARE_VIEW=='popup') { ?>onClick="window.open('<?php echo tep_href_link(FILENAME_COMPARE_PRINT, 'cc='.$compare_category, 'NONSSL'); ?>', '<?php echo $category_info['categories_name'].' '.Translate('vergelijken');?>','scrollbars=yes,resizable=yes,status=yes,width=800,height=600'); return false"<?php } ?> target="_blank" id="compare_heading" title="<?php echo Translate('Klik om de geselecteerde producten te vergelijken'); ?>"><?php echo $lc_text; ?></a></span>
                                <?php
								...
								<?php } elseif (PRODUCT_COMPARE_VIEW=='lightbox') { ?>
									...
									/* <![CDATA[ */
									jQuery(document).ready(function(){
										jQuery("#compare_heading").colorbox({href:"<?php echo tep_href_link(FILENAME_AJAX_SEARCH, 'mode=compare&cc='.$compare_category); ?>",width:"850px",height:"85%"});							 
									});
									/* ]]> */
									</script>
								<?php }
						...
						if ($list_config['configuration_value']>0) {
							...
							<div class="column column-<?php echo $column_list.$column_class; ?>"<?php if ($display_class=='list') { ?> style="width:<?php echo $column_width; ?>%;"<?php } ?>>
								...
								<?php
									...
									if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_MODEL') {
										?>
										<div class="product-model"><a href="<?php echo tep_href_link(FILENAME_PRODUCT_INFO, 'products_id='.$listing['products_id']); ?>"><?php echo $listing['products_model']; ?></a></div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_NAME') {
										?>
										<div class="product-name"><h2><a href="<?php echo tep_href_link(FILENAME_PRODUCT_INFO, 'products_id='.$listing['products_id']); ?>"><?php echo $listing['products_name']; ?></a></h2></div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_DESCRIPTION') {
										?>
										<div class="product-description"><?php echo substr($listing['products_description'], 0, 100); ?></div>
										<?php	
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_TECHNICAL') {
										?>
										<div class="product-technical"><?php echo $listing['products_technical']; ?></div>
										<?php	
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_MANUFACTURER') {
										?>
										<div class="product-manufacturer"><?php echo $listing['manufacturers_name']; ?></div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_QUANTITY') {
										...
										if ($voorraad>0) {
										?>
										<div class="product-quantity available"><?php echo Translate('Beschikbaar'); ?></div>
										<?php
										} else {
										?>
										<div class="product-quantity unavailable"><?php echo Translate('Niet beschikbaar'); ?></div>
										<?php
										}
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_IMAGE') {
										...
										?>
										<div class="product-image"><a href="<?php echo tep_href_link(FILENAME_PRODUCT_INFO, 'products_id='.$listing['products_id']); ?>"><?php echo tep_image(DIR_WS_IMAGES.$listing['products_image'], $listing['products_name'], $image_width, $image_height); ?></a></div>
										<?php
										echo tep_get_product_stars($listing['products_id'], $display_class, $discount);
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_WEIGHT') {
										?>
										<div class="product-quantity"><?php echo $listing['products_weight']; ?></div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_PRICE') {
										//DISCOUNT
										if ($discount_price['lowest']['discount'] > 0 && PRICE_BOOK == 'true') {
											if (tep_not_null($listing['specials_new_products_price'])) {
												...
												if ($new_price < $discount_price['lowest']['price']) {
													...
													$products_price .= $currencies->display_price($new_price, tep_get_tax_rate($listing['products_tax_class_id']));
													$products_price .= '</span>';
												} else {
													$products_price = '<span class="oldprice">';
													$products_price .= $currencies->display_price($listing['products_price'], tep_get_tax_rate($listing['products_tax_class_id']));
													$products_price .= '</span><br />';
													$products_price .= '<span class="specialprice">';
													$products_price .= $currencies->display_price($discount_price['lowest']['price'], tep_get_tax_rate($listing['products_tax_class_id']));
													$products_price .= '</span>';
												}
											} else {
												$products_price = '<span class="oldprice">';
											...
											if (tep_not_null($listing['specials_new_products_price'])) {
												$products_price = '<span class="oldprice">' . $currencies->display_price($listing['products_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</span> <span class="specialprice">' . $currencies->display_price($listing['specials_new_products_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</span>';
											} else {
												$products_price = '<span class="yourprice">' . $currencies->display_price($listing['products_price'], tep_get_tax_rate($listing['products_tax_class_id'])).'</span>';
											}
										}
...
/**************** #1224 check product_quantity ************/

$product_info_query = tep_db_query("select p.products_id, pd.products_name, pd.products_description, p.products_model, p.products_quantity, p.products_image, pd.products_url, p.products_price, p.products_tax_class_id, p.products_date_added, p.products_date_available, p.manufacturers_id, pd.products_technical, p.products_image_1, p.products_image_2, p.products_image_3, p.products_image_4, p.products_opt1, p.products_opt2, p.products_opt3, p.products_opt4, p.products_opt5 from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$listing['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
$product_info = tep_db_fetch_array($product_info_query);

/*****************************/
										?>
										<div class="product-buy-now">
<?php if((STOCK_ALLOW_CHECKOUT == 'true')||($product_info['products_quantity'] >0 )){?>
										
<a href="<?php echo tep_href_link(FILENAME_PRODUCT_INFO, 'products_id='.$listing['products_id']); ?>"class="button-a"><?php echo Translate('Voeg toe aan winkelwagen'); ?></a> <?php }else{
echo Translate("geen voorraad");
}?>
										<!-- Original changed for #1224 -- <a href="<?php// echo tep_href_link(FILENAME_DEFAULT, tep_get_all_get_params(array('action')).'action=buy_now&products_id='.$listing['products_id']); ?>" class="button-a"><?php// echo Translate('Voeg toe aan winkelwagen'); ?></a>-->
										</div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT1') {
										?>
										<div class="product-opt products_opt1"><?php echo $listing['products_opt1']; ?></div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT2') {
										?>
										<div class="product-opt products_opt2"><?php echo $listing['products_opt2']; ?></div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT3') {
										?>
										<div class="product-opt products_opt3"><?php echo $listing['products_opt3']; ?></div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT4') {
										?>
										<div class="product-opt products_opt4"><?php echo $listing['products_opt4']; ?></div>
										<?php
									} else if ($list_config['configuration_key']=='PRODUCT_'.$listtype.'_OPT5') {
										?>
										<div class="product-opt products_opt5"><?php echo $listing['products_opt5']; ?></div>
										<?php
									} else if (($list_config['configuration_key']=='PRODUCT_'.$listtype.'_COMPARE') && (PRODUCT_COMPARE=='true') && ($listing_type=='product_listing') && ($_GET['cPath']!='')) {
										?>
										<div class="product-opt products_compare">
                                        	<?php
											$is_checked = '';
											$compare_category = end(explode('_', $_GET['cPath']));
											if ($_COOKIE['compare_'.$compare_category]) {
												$cookie_val = $_COOKIE['compare_'.$compare_category];
												if (strstr($cookie_val, '_'.$listing['products_id'])) {
													$is_checked = ' checked';
												}
											}
											?>
                                        	<input type="checkbox" id="compare_<?php echo $listing['products_id']; ?>" name="<?php echo $listing['products_name']; ?>"<?php echo $is_checked; ?> />
                                        </div>
										<?php
									}
								?>
									...
									for ($s=0 ; $s<$specs_col_count; $s++) {
										...
										<div class="column column-<?php echo $column_list.$column_class; ?>"<?php if ($display_class=='list') { ?> style="width:<?php echo $column_width; ?>%;"<?php } ?>>
											<div class="column_inner">
												...
												echo show_product_spec($listing['products_id'], $specification_group, $subkenmerk, $display_class);
												?>
												</div>
											</div>
										</div>
	...
	if (($listing_type=='product_listing') || ($listing_type=='specials_listing') || ($listing_type=='products_new_listing') || ($listing_type=='search_results')) {
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="2">
		  <tr>
			...
			<td class="smallTextpaging pages" align="right"><?php echo Translate('Paginas') . ': ' . $listing_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></td>
		  </tr>
		</table>
		<?php
		}
