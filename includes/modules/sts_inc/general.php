...
switch (true) {
	...
	case (basename($_SERVER['PHP_SELF']) === FILENAME_PRODUCT_PRINT): 
		...
		if ($_GET['products_id'] != '') {
		   ...
		   $canonical_url = tep_href_link(basename($_SERVER['PHP_SELF']), 'products_id='.(int)$_GET['products_id'], 'NONSSL', false);
		...
		if (count($defaultKeywords) >= 20) {
			...
			if (count($allKeywords) > 20) {
				...
				for ($i=0;$i<20;$i++) {
					...
					if($allKeywords[$i]) $keywords_product .= $allKeywords[$i].', ';
				...
				foreach( $allKeywords as $cld => $separated ) {
					...
					if($separated) $keywords_product .= $separated.', ';
		...
		$headertags_extra_css .= ' <link rel="stylesheet" type="text/css" href="'.substr(((($request_type == 'SSL') ? DIR_WS_HTTPS_CATALOG : DIR_WS_HTTP_CATALOG) . STS_TEMPLATE_DIR),0,-1).'/css/jquery.rating.css" />' . "\n";
		$headertags_extra_css .= ' <link rel="stylesheet" type="text/css" href="'.substr(((($request_type == 'SSL') ? DIR_WS_HTTPS_CATALOG : DIR_WS_HTTP_CATALOG) . STS_TEMPLATE_DIR),0,-1).'/css/style_review.css" />' . "\n";
	...
	case (basename($_SERVER['PHP_SELF']) === FILENAME_CONDITIONS):
		...
		if ($_GET['page'] != '') {
		   ...
		   $canonical_url = tep_href_link(basename($_SERVER['PHP_SELF']), 'page='.(int)$_GET['page'], 'NONSSL', false);
		...
		if (count($defaultKeywords) >= 20) {
			...
			for ($i=0;$i<$count_extra_words;$i++){
				...
				if($std_keywords[$i]) $keywords_product .= $std_keywords[$i].', ';
	...
	case (basename($_SERVER['PHP_SELF']) === FILENAME_SHOPPING_CART):
		...
		if ((basename($_SERVER['PHP_SELF']) === FILENAME_ACCOUNT) || (basename($_SERVER['PHP_SELF']) === FILENAME_ACCOUNT_EDIT) || (basename($_SERVER['PHP_SELF']) === FILENAME_ACCOUNT_NEWSLETTERS) || (basename($_SERVER['PHP_SELF']) === FILENAME_ACCOUNT_NOTIFICATIONS) || (basename($_SERVER['PHP_SELF']) === FILENAME_ACCOUNT_PASSWORD)) {
			...
			$headertags_extra_css .= ' <link rel="stylesheet" type="text/css" href="'.substr(((($request_type == 'SSL') ? DIR_WS_HTTPS_CATALOG : DIR_WS_HTTP_CATALOG) . STS_TEMPLATE_DIR),0,-1).'/css/style_checkout.css" />' . "\n";
	...
	case ((basename($_SERVER['PHP_SELF']) === FILENAME_DEFAULT || $_SERVER['PHP_SELF'] == '/') && (!isset($_GET['cPath'])) && (!isset($_GET['manufacturers_id']))):
	   ...
	   $canonical_url = tep_href_link(FILENAME_DEFAULT, '', 'NONSSL', false);
		...
		$headertags_extra_css .= ' <link rel="stylesheet" type="text/css" href="'.substr(((($request_type == 'SSL') ? DIR_WS_HTTPS_CATALOG : DIR_WS_HTTP_CATALOG) . STS_TEMPLATE_DIR),0,-1).'/css/style_home.css" />' . "\n";
		...
		if (count($defaultKeywords) >= 20) {
			...
			if (count($allKeywords) > 20) {
				...
				for ($i=0;$i<20;$i++) {
					...
					if($allKeywords[$i]) $keywords_category .= $allKeywords[$i].', ';
				...
				foreach( $allKeywords as $cld => $separated ) {
					...
					if($separated) $keywords_category .= $separated.', ';
	...
	case ((basename($_SERVER['PHP_SELF']) === FILENAME_DEFAULT) && (isset($_GET['manufacturers_id'])) && ($_GET['manufacturers_id']!='')):
		...
		if ($_GET['manufacturers_id'] != '') {
		   ...
		   $canonical_url = tep_href_link(basename($_SERVER['PHP_SELF']), 'manufacturers_id='.(int)$_GET['manufacturers_id'], 'NONSSL', false);
		...
		if (count($defaultKeywords) >= 20) {
			...
			if (count($allKeywords) > 20) {
				...
				for ($i=0;$i<20;$i++) {
					...
					if($allKeywords[$i]) $keywords_manufacturer .= $allKeywords[$i].', ';
				...
				foreach( $allKeywords as $cld => $separated ) {
					...
					if($separated) $keywords_manufacturer .= $separated.', ';
...
$sts->template['headertags'] .= ' <script type="text/javascript" src="'.substr(((($request_type == 'SSL') ? DIR_WS_HTTPS_CATALOG : DIR_WS_HTTP_CATALOG) . STS_TEMPLATE_DIR),0,-1).'/js/plugins/form_validation.js"></script>' . "\n";
