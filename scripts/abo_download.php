...
if ( $download == 'stream' or  $download == 'tempfile' ){
	...
	if ($download == 'stream'){
		...
		fclose($fp);
		$sql = 'UPDATE orders SET orders_status=2 WHERE orders_status=1';
...
<p class="smallText">
	  ...
	  <a href="orders_download.php?download=tempfile&dltype=full&orders_status=1">Download <b>Pending</b> orders</a><br>
	  </td>
	</tr>
...
if ($orders_status){
		$filelayout_sql = "SELECT
			...
			WHERE
			o.orders_status= " . tep_db_input($orders_status) . " 
			AND
