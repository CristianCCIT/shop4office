...
/*
  $Id: ABO_orderoptions.php,v 1.0 2004/08/01 chris@aboservice.be Exp $
*/
...
if ( $download == 'stream' or  $download == 'tempfile' ){
	...
	if ($download == 'stream'){
		...
		fclose($fp);
//		$sql = 'UPDATE orders SET orders_status= ' . $orders_status . ' WHERE orders_status= 99';
...
<p class="smallText">
	  ...
	  <a href="abo_orderoptions.php?download=tempfile&dltype=full&orders_status=99">Download <b>Pending</b> orders</a><br>
	  </td>
	</tr>
...
if ($orders_status){
		...
		$filelayout_sql = "SELECT
			...
			ot.orders_products_id as prod_id,
                        ot.products_options as prod_option,
                        ot.products_options_values as option_value ,
			ot.options_values_price as value_price
			FROM ".TABLE_ORDERS." as o ,  ".TABLE_ORDERS_PRODUCTS_ATTRIBUTES." as ot
			WHERE
			o.abo_status=99 AND o.orders_id = ot.orders_id ;";
}else{
		$filelayout_sql = "SELECT
			o.orders_id as orders_id,
			ot.orders_products_id as prod_id,
