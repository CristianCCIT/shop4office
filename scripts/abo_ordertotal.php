...
if ( $download == 'stream' or  $download == 'tempfile' ){
	...
	if ($download == 'stream'){
		...
		fclose($fp);
		$sql = 'UPDATE orders SET abo_status=1, orders_status="'.$orders_status.'" WHERE abo_status=99';
...
<p class="smallText">
	  ...
	  <a href="abo_ordertotal.php?download=tempfile&dltype=full&orders_status=99">Download <b>Pending</b> orders</a><br>
	  </td>
	</tr>
...
if ($orders_status){
		...
		$filelayout_sql = "SELECT
			...
			ot.title as title,
                        ...
                        ot.value as value ,
			ot.class as class
			FROM ".TABLE_ORDERS." as o ,  ".TABLE_ORDERS_TOTAL." as ot
			WHERE
