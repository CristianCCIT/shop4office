...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function tep_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false, $disable_with_subs = false, $disable_with_prods = false) {
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
      if ($exclude != $categories['categories_id']) {
		  ...
		  } else if ($disable_with_prods == true) {
  ...
  function tep_draw_products_pull_down($name, $parameters = '', $exclude = '') {
    global $currencies, $languages_id, $c2m_list;
    ...
    $select_string .= '>';
	if ($c2m_list) { $products_limit = "	AND (p.manufacturers_id, p2c.categories_id) NOT IN (".$c2m_list.")"; } else { $products_limit = ""; }
    $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_price from " . TABLE_PRODUCTS . " p JOIN ".TABLE_PRODUCTS_TO_CATEGORIES." p2c USING (products_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = 1 AND p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'".$products_limit." order by products_name");
...
// BOF: Radders - Automatic Thumbnail Creator // Modded by MaxiDVD 22/01/2004 MS2-3 Admin Check.
  function tep_info_image($image, $alt, $width = '', $height = '') {
  $image = tep_use_resampled_image_list($image,$width,$height);
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES . $image)) ) {
      $image = tep_image(DIR_WS_CATALOG_IMAGES . $image, $alt, $width, $height);
    } else {
      $image = TEXT_IMAGE_NONEXISTENT;
  ...
  function tep_use_resampled_image_list($image,$width,$height) {
        if ($image=='') {
                return $image;
         }
        if(! ($i = @getimagesize( DIR_FS_DOCUMENT_ROOT . DIR_WS_IMAGES . $image ))) {
                return $image; // Can amend to work with other images.
        ...
        $file = eregi_replace( '\.([a-z]{3,4})$', "-{$width}x{$height}.\\1", $image );  // Name of resampled image.
        if (is_file( DIR_FS_DOCUMENT_ROOT . DIR_WS_IMAGES . $file ) ) {
                return $file;
        }
        return $image;
      }
...
// Text Mode
  ...
  function tep_get_languages($use_id_as_key = false) {
    $languages_query = tep_db_query("select languages_id, name, code, image, directory from " . TABLE_LANGUAGES . " order by sort_order", 'db_link_local');
    $i = -1;
...
// Sets the status of a product on special
  function tep_set_specials_status($specials_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_SPECIALS . " set status = '1', expires_date = NULL, date_status_change = NULL where specials_id = '" . (int)$specials_id . "'", 'db_link_local');
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_SPECIALS . " set status = '0', date_status_change = now() where specials_id = '" . (int)$specials_id . "'", 'db_link_local');
    } else {
      return -1;
    }
  }

////rmh 01/05/2004 vendor locator function tep_set_vendor_status()
// Sets the status of a vendor
  function tep_set_vendor_status($vendors_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_VENDORS . " set vendors_status = '1' where vendors_id = '" . (int)$vendors_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_VENDORS . " set vendors_status = '0' where vendors_id = '" . (int)$vendors_id . "'");
    } else {
      return -1;
    }
  }

// Sets the vendor, as a place where customer can get prize
  function tep_set_vendor_prize_place($vendors_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_VENDORS . " set vendors_prize_get_place = '1' where vendors_id = '" . (int)$vendors_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_VENDORS . " set vendors_prize_get_place = '0' where vendors_id = '" . (int)$vendors_id . "'");
    } else {
      return -1;
    }
  }
...
// Retreive server information
  ...
  function tep_remove_category($category_id) {
	...
	if( tep_db_num_rows( tep_db_query("SHOW TABLES LIKE 'seo_urls'"))) {
		tep_db_query("DELETE FROM `seo_urls` WHERE categories_id = '".$category_id."'");
	}
  ...
  function tep_remove_product($product_id) {
	...
	if( tep_db_num_rows( tep_db_query("SHOW TABLES LIKE 'seo_urls'"))) {
		tep_db_query("DELETE FROM `seo_urls` WHERE products_id = '".$product_id."'");
	}
...
// Calculates Tax rounding the result
  ...
  function tep_remove_tax($price, $tax) {
    global $currencies;
	
	return tep_round($price / ((100+$tax)/100), $currencies->currencies[DEFAULT_CURRENCY]['decimal_places']);
  }
...
function reset_cache_dir($dir)
{
  ...
  function tep_draw_pull_down_menu_sites($name, $values, $default = '', $parameters = '', $required = false) {
   ...
   for ($i=0, $n=sizeof($values); $i<$n; $i++) {
      ...
      if( $splitted = explode( ';', $default ) )
		{
			foreach( $splitted as $cle => $separated )
			{
			  if ($separated == $values[$i]['id']) {
				$field .= ' SELECTED';
			  }
			}
		}
  ...
  function tep_draw_pull_down_menu_categories($name, $values, $default = '', $parameters = '', $required = false) {
    $field = '<select name="' . tep_output_string($name) . '"';
...
function check_admin_modules($module) {
	$config_query = tep_db_query("SELECT * FROM config WHERE configurations_key = '".$module."'", 'db_link_local');
	$config = tep_db_fetch_array($config_query);
	return $config['configurations_value'];
}

function tep_count_products_in_category($cat_id) {
	if ($cat_id != "") {
		$query = tep_db_query('SELECT COUNT(products_id) FROM products_to_categories WHERE categories_id = "'.$cat_id.'"');
		$products = tep_db_fetch_array($query);
		return $products['COUNT(products_id)'];
	} else {
		return '0';
	}
}
...
function encrypt($sData, $sKey='abo'){ 
    $sResult = ''; 
...
function decrypt($sData, $sKey='abo'){ 
    $sResult = ''; 
