...
if (!isset($products_per_search_page) || !is_integer($products_per_search_page) || ($products_per_search_page < 1)) {
	$products_per_search_page = $query_limit = MAX_DISPLAY_SEARCH_RESULTS;
}
if (isset($_GET['ppp']) && is_numeric($_GET['ppp']) && ($_GET['ppp'] > 0)) {
	$products_per_search_page = $query_limit = intval($_GET['ppp']);
}
$ppp_list = array();
for ($i = 1; $i <= 10; $i++) {
...
if ($listing_split->number_of_rows > 0) {
	...
	if ($total_columns > 0) {
		...
		<div class="panel panel-info">
			<?php if ($listing_type == 'home_products') { ?>
